export function createGameClockManager({
  getState,
  getUi,
  pad,
  clockMaxMinutes,
  ...
  function setClock() {
    ...
    const m = Number.isFinite(rawMinutes)
      ? Math.max(0, Math.min(rawMinutes, clockMaxMinutes))
      : 0;
    const s = Number.isFinite(rawSeconds)
      ? Math.max(0, Math.min(rawSeconds, 59))
      : 0;
