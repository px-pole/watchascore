...
function isSafeSvgAttribute(attribute) {
  ...
  if (!SVG_ATTRIBUTES.has(name) || /^on/i.test(name)) return false;
  if (/(javascript|data:|vbscript|expression|url\s*\()/i.test(value)) {
...
export function sanitizeSvgText(svgText) {
  ...
  const elements = [
    ...
    ...parsed.documentElement.querySelectorAll("*"),
  ];
...
function svgDataUrlToText(dataUrl) {
...
`;
  ...
  const brightnessWorker = new Worker(workerUrl);
  ...
  function getCropSettings() {
    ...
    const zoomPercent = clamp(
      parseFloat(ui.cropZoom?.value || "100"),
      100,
      300,
    );
  ...
  function syncCropControlLabels() {
    ...
    if (ui.cropZoomValue)
      ui.cropZoomValue.textContent = `${Math.round(zoomPercent)}%`;
    if (ui.cropTopValue)
      ui.cropTopValue.textContent = `${Math.round(topPercent)}%`;
    if (ui.cropRightValue)
      ui.cropRightValue.textContent = `${Math.round(rightPercent)}%`;
    if (ui.cropBottomValue)
      ui.cropBottomValue.textContent = `${Math.round(bottomPercent)}%`;
    if (ui.cropLeftValue)
      ui.cropLeftValue.textContent = `${Math.round(leftPercent)}%`;
  }
  ...
  function getCropRect(
    ...
    const trimmedWidth = Math.max(
      1,
      width * (1 - (leftPercent + rightPercent) / 100),
    );
    const trimmedHeight = Math.max(
      1,
      height * (1 - (topPercent + bottomPercent) / 100),
    );
  ...
  function renderCropPreview() {
    ...
    const { zoom, topPercent, rightPercent, bottomPercent, leftPercent } =
      getCropSettings();
    ...
    if (!pendingAnalysis.has(src)) {
      ...
      try {
        ...
        brightnessWorker.postMessage(
          { imageData: imageData.data.buffer, src },
  ...
  function setBadge(side, src) {
    ...
    const badgeConfigs = [
      ...
      {
        img: ui[`mini${sideKey}Badge`],
        wrap: ui[`mini${sideKey}BadgeWrap`],
      },
    ];
  ...
  function handleLogoUpload(side, input) {
    ...
    if (file.size > maxSizeMb * 1024 * 1024) {
      alert(
        `The selected image is too large. Please upload a file smaller than ${maxSizeMb}MB.`,
      );
      input.value = "";
      return;
    }
    ...
    reader.onload = (e) => {
      ...
      sourceImg.onload = () => {
        if (sourceImg.naturalWidth > 4096 || sourceImg.naturalHeight > 4096) {
          alert("The selected image dimensions are too large.");
          ...
          onClose: () => {
            ...
            if (previewCtx && ui.cropPreviewCanvas) {
              previewCtx.clearRect(
                0,
                0,
                ui.cropPreviewCanvas.width,
                ui.cropPreviewCanvas.height,
              );
            }
          },
        });
      ...
      sourceImg.onerror = () => {
        alert(
          "Could not load this image for cropping. Please try another file.",
        );
        clearPendingLogoState();
      };

      if (file.type === "image/svg+xml") {
        const svgText = svgDataUrlToText(pendingLogoSource);
        const sanitizedSvg = svgText ? sanitizeSvgText(svgText) : null;
        if (!sanitizedSvg) {
          alert("The SVG logo could not be safely sanitized.");
          clearPendingLogoState();
          input.value = "";
          return;
        }
        pendingLogoSource = `data:image/svg+xml;charset=utf-8,${encodeURIComponent(sanitizedSvg)}`;
      }
