...
export function buildPenaltyMarkSlots(state, side, maxVisibleSlots = 5) {
  ...
  visibleAttempts.forEach((result, index) => {
    const visualIndex = side === 'home'
      ? (maxVisibleSlots - 1 - index)
      : index;

    orderedSlots[visualIndex] = {
      result,
      absoluteAttemptNumber: attempts.length - visibleAttempts.length + index + 1
    };
  });
...
export function getPenaltyScore(state, side) {
  return getPenaltyAttempts(state, side).reduce((total, result) => total + (result === 'made' ? 1 : 0), 0);
}
...
export function buildUndoLastPenaltyUpdate(state) {
  ...
  return buildUndoPenaltyUpdate(state, 'away') || buildUndoPenaltyUpdate(state, 'home');
}