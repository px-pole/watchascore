export function createScoreboardUiManager({
  ...
  function checkWrapperState() {
    ...
    const currentlySelectedClass = ui.wrapper.classList.contains('teams-selected');
  ...
  function updateScoreUI(side, value) {
    ...
    sides.forEach((currentSide) => {
      ...
      const score = (side === currentSide && value !== undefined) ? value : state[`${currentSide}Score`];
  ...
  function updateClockUI() {
    ...
    if (ui.clockWrap) {
      ui.clockWrap.classList.toggle('is-hidden', state.clockVisible === false || state.penaltyMode === true);
    }
  ...
  function syncScoreboardLayoutState() {
    ...
    if (penaltyMode) {
      layoutClass = teamNamesHidden ? 'layout-penalty-hidden-names' : 'layout-penalty';
  ...
  function updatePenaltyUI() {
    ...
    if (ui.togglePenaltyModeBtn) {
      ...
      ui.togglePenaltyModeBtn.setAttribute('aria-pressed', penaltyMode ? 'true' : 'false');
    ...
    if (ui.toggleClockBtn) {
      ...
      ui.toggleClockBtn.setAttribute('aria-hidden', penaltyMode ? 'true' : 'false');
    }
    ...
    if (ui.penaltyControls) {
      ...
      ui.penaltyControls.setAttribute('aria-hidden', penaltyMode ? 'false' : 'true');
    }

    if (ui.normalScoreControls) {
      ui.normalScoreControls.style.display = penaltyMode ? 'none' : '';
      ui.normalScoreControls.setAttribute('aria-hidden', penaltyMode ? 'true' : 'false');
    }
    ...
    if (ui.ctrlHomePenaltyScore) ui.ctrlHomePenaltyScore.textContent = homePenaltyScore;
    if (ui.ctrlAwayPenaltyScore) ui.ctrlAwayPenaltyScore.textContent = awayPenaltyScore;
    ...
    if (ui.penaltyTracker) {
      ...
      ui.penaltyTracker.setAttribute('aria-hidden', penaltyMode ? 'false' : 'true');
    }
  ...
  function syncPenaltyTightSpace() {
    ...
    const penaltyMode = state.penaltyMode === true;
    if (!penaltyMode || !ui.penaltyTracker || !ui.homeBadgeWrap || !ui.awayBadgeWrap) {
  ...
  function renderPenaltyMarks(side) {
    ...
    slots.forEach(({ result, slotNumber, absoluteAttemptNumber }) => {
      ...
      if (!result) {
        ...
        mark.setAttribute('aria-label', `${capitalize(side)} penalty ${absoluteAttemptNumber}: pending`);
      } else {
        mark.className = `penalty-mark ${isMade ? 'is-made' : 'is-missed'}`;
        mark.textContent = isMade ? '✓' : '✕';
        mark.setAttribute('aria-label', `${capitalize(side)} penalty ${absoluteAttemptNumber}: ${isMade ? 'scored' : 'missed'}`);
      }
