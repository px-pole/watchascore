export function createScoreboardUiManager({
  ...
  function updateTeamsUI() {
    ...
    if (ui.tournamentTitleOverride) {
      const homeLeague = state.homeTeam?.league || "";
      const awayLeague = state.awayTeam?.league || "";
      const crossLeague = !!(
        homeLeague &&
        awayLeague &&
        homeLeague !== awayLeague &&
        !getTrimmedValue(state, "tournamentTitleOverride")
      );
      ui.tournamentTitleOverride.classList.toggle(
        "cross-league-hint",
        crossLeague,
      );
      if (crossLeague) {
        ui.tournamentTitleOverride.title =
          "Teams are from different leagues — consider setting a custom title.";
      } else {
        ui.tournamentTitleOverride.removeAttribute("title");
      }
    }
  }
