export function createTeamSearchManager({
	...
	function renderTeamOption(
		side,
		team,
		resultsDiv,
		searchInput,
		highlightedName = team.name,
	) {
	...
	function renderSearchMode(fragment, side, filter, resultsDiv, searchInput) {
		...
		filteredByLeague.forEach((teams, leagueName) => {
			...
			teams.forEach((t) => {
				...
				fragment.appendChild(
					renderTeamOption(side, t, resultsDiv, searchInput, highlightedName),
				);
			});
		});
		...
		if (foundCount === 0) {
			...
			none.innerHTML =
				'<i class="fa-solid fa-magnifying-glass-question"></i><span>No teams matched your search</span>';
		...
		orderedLeagues.forEach(({ leagueName, teams }) => {
			...
			teams.forEach((t) => {
				teamContainer.appendChild(
					renderTeamOption(side, t, resultsDiv, searchInput),
				);
			});
