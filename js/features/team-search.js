export function createTeamSearchManager({
  ...
  function prepareTeamData() {
    ...
    Object.keys(source)
      .sort()
      .forEach((leagueName) => {
        const sortedTeams = [...source[leagueName]].sort((a, b) =>
  ...
  function positionSearchPopup(resultsDiv, searchInput) {
    ...
    const visibleRows = Array.from(
      resultsDiv.querySelectorAll(
        ".search-results-item, .search-results-header, .search-results-none",
      ),
    ...
    const maxPopupHeight = Math.floor(
      Math.max(
        140,
        Math.min(availableSpace, viewportHeight * 0.75, rowLimitedHeight),
      ),
    );
  ...
  function refreshNavigableItems(side, resultsDiv) {
    const navItems = Array.from(
      resultsDiv.querySelectorAll(
        ".search-results-item, .search-results-header.collapsible",
      ),
    ).filter((item) => item.offsetParent !== null);
    sideSearchState[side].navItems = navItems;
    sideSearchState[side].currentIndex = navItems.findIndex((item) =>
  ...
  function toggleLeagueHeader(resultsDiv, header) {
    ...
    const teamContainer = containerId
    ...
    if (!wasOpen && teamContainer.dataset.rendered !== "true") {
      const league = teamsByLeague.find(
        ({ leagueName }) => leagueName === header.dataset.league,
      );
      league?.teams.forEach((team) => {
        teamContainer.appendChild(
          renderTeamOption(side, team, resultsDiv, searchInput),
        );
      });
      teamContainer.dataset.rendered = "true";
    }
    const anchorTop = header.getBoundingClientRect().top;
    const openContainers = Array.from(
      resultsDiv.querySelectorAll(".league-items-container.active"),
    );
    const switchingGroup =
      !wasOpen &&
      openContainers.some((container) => container !== teamContainer);
    ...
    resultsDiv
      .querySelectorAll(
        '.search-results-header.collapsible[aria-expanded="true"]',
      )
      .forEach((openHeader) => {
    ...
    if (isOpen) {
      ...
      const otherResultsDiv = document.getElementById(
        `${otherSide}-team-results`,
      );
      const otherSearchInput = document.getElementById(
        `${otherSide}-team-search`,
      );
      const canMirror =
        otherResultsDiv &&
        otherSearchInput &&
      ...
      if (canMirror) {
        const targetHeader = Array.from(
          otherResultsDiv.querySelectorAll(
            ".search-results-header.collapsible",
          ),
        ).find((candidate) => candidate.dataset.league === mirroredLeagueName);
        if (targetHeader) {
          ...
          if (targetContainer) {
            const openedContainers = Array.from(
              otherResultsDiv.querySelectorAll(
                ".league-items-container.active",
              ),
            );
            const switchingInOther = openedContainers.some(
            ...
            if (targetContainer.dataset.rendered !== "true") {
              const league = teamsByLeague.find(
                ({ leagueName }) => leagueName === targetHeader.dataset.league,
              );
              league?.teams.forEach((team) => {
              ...
              targetContainer.dataset.rendered = "true";
            }
            targetHeader.setAttribute("aria-expanded", "true");

            if (otherResultsDiv.firstElementChild !== targetHeader) {
              otherResultsDiv.insertBefore(
                targetHeader,
                otherResultsDiv.firstElementChild,
              );
              otherResultsDiv.insertBefore(
                targetContainer,
                targetHeader.nextSibling,
              );
            }
  ...
  function renderTeamOption(side, team, resultsDiv, searchInput, filter = "") {
    ...
    const badgeSrc =
      searchPopupBadgeOverrides.get(team.id) || team.badge || placeholder;
    ...
    const matchIndex = filter ? team.nameLower.indexOf(filter) : -1;
    if (matchIndex >= 0) {
      name.append(document.createTextNode(team.name.slice(0, matchIndex)));
      const highlight = document.createElement("span");
      highlight.className = "search-highlight";
      highlight.textContent = team.name.slice(
        matchIndex,
        matchIndex + filter.length,
      );
      name.append(highlight);
      name.append(
        document.createTextNode(team.name.slice(matchIndex + filter.length)),
      );
    } else {
  ...
  function renderSearchMode(fragment, side, filter, resultsDiv, searchInput) {
    ...
    for (const team of allTeams) {
      const subMatch =
        team.nameLower.includes(filter) || team.idLower.includes(filter);
      if (subMatch) {
    ...
    if (!isCapped && filter.length > 3) {
      for (const team of allTeams) {
        ...
        const fuzzyMatch = team.nameWordsLower.some(
          (word) =>
    ...
    filteredByLeague.forEach((teams, leagueName) => {
      ...
      teams.forEach((t) => {
        fragment.appendChild(
          renderTeamOption(side, t, resultsDiv, searchInput, filter),
        );
      });
    });
    ...
    if (foundCount === 0) {
      ...
      icon.className = "fa-solid fa-magnifying-glass-question";
      icon.setAttribute("aria-hidden", "true");
      const message = document.createElement("span");
      message.textContent = "No teams matched your search";
      none.append(icon, message);
  ...
  function renderBrowseMode(fragment, side, resultsDiv, searchInput) {
    const orderedLeagues = mirroredLeagueName
      ? [
          ...teamsByLeague.filter(
            ({ leagueName }) => leagueName === mirroredLeagueName,
          ),
          ...teamsByLeague.filter(
            ({ leagueName }) => leagueName !== mirroredLeagueName,
          ),
  ...
  const debouncedSearch = debounce((side, text) => {
    ...
    if (
      ...
      resultsDiv.childElementCount > 0
    ) {
  ...
  function repositionActivePopups(getUi) {
    document
      .querySelectorAll(".search-results-popup.active")
      .forEach((popup) => {
