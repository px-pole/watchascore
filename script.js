...
const normalizeTeam = (team) => {
  if (!team || typeof team !== "object" || typeof team.id !== "string")
    return null;

  const badge = typeof team.badge === "string" ? team.badge : "";
  const safeBadge =
    badge.startsWith("assets/") || badge.startsWith("data:image/") ? badge : "";

  return {
    id: team.id.slice(0, 120),
    name: typeof team.name === "string" ? team.name.slice(0, 120) : "",
    league: typeof team.league === "string" ? team.league.slice(0, 120) : "",
...
const normalizeState = (candidate) => {
  ...
  const normalizePenalties = (value) =>
    Array.isArray(value)
      ? value
          .filter((result) => result === "made" || result === "missed")
          .slice(-100)
      : [];

  return {
    ...INITIAL_STATE,
    homeScore: clampInteger(source.homeScore, 0, 9999, 0),
    awayScore: clampInteger(source.awayScore, 0, 9999, 0),
    penaltyMode: source.penaltyMode === true,
    homePenalties: normalizePenalties(source.homePenalties),
    awayPenalties: normalizePenalties(source.awayPenalties),
    penaltyHistory: Array.isArray(source.penaltyHistory)
      ? source.penaltyHistory
          .filter((side) => side === "home" || side === "away")
          .slice(-200)
      : [],
    clockSec: clampInteger(source.clockSec, 0, CLOCK_MAX_MINUTES * 60 + 59, 0),
    running: false,
    status: typeof source.status === "string" ? source.status.slice(0, 30) : "",
    homeTeam: normalizeTeam(source.homeTeam),
    awayTeam: normalizeTeam(source.awayTeam),
    theme:
      THEMES.includes(source.theme) || source.theme === "default"
        ? source.theme
        : "default",
    homeNameOverride:
      typeof source.homeNameOverride === "string"
        ? source.homeNameOverride.trim().slice(0, 120)
        : "",
    awayNameOverride:
      typeof source.awayNameOverride === "string"
        ? source.awayNameOverride.trim().slice(0, 120)
        : "",
    mode: source.mode === INITIAL_STATE.mode ? source.mode : INITIAL_STATE.mode,
    visibilityMode: ["none", "glow", "contrast"].includes(source.visibilityMode)
      ? source.visibilityMode
      : "none",
    tournamentTitleOverride:
      typeof source.tournamentTitleOverride === "string"
        ? source.tournamentTitleOverride.trim().slice(0, 120)
        : "",
    startTime:
      typeof source.startTime === "string"
        ? source.startTime.slice(0, 30)
        : null,
    clockVisible: source.clockVisible !== false,
    teamNamesVisible: source.teamNamesVisible !== false,
  };
};
...
const stateBatchManager = createStateBatchManager({
  ...
  shouldSave: (prop, snapshot) =>
    prop !== "clockSec" || snapshot.running !== true,
});
...
window.addEventListener("pagehide", () => mediaManager.dispose(), {
  once: true,
});
...
const teamNamesManager = createTeamNamesManager({
  getState,
  getUi,
  capitalize,
});
...
function init() {
  ...
  if (typeof rawData.teamNamesVisible !== "boolean")
    rawData.teamNamesVisible = true;
  if (!Array.isArray(rawData.penaltyHistory)) rawData.penaltyHistory = [];
  if (rawData.theme !== "default" && !THEMES.includes(rawData.theme))
    rawData.theme = "default";
...
function cacheElements() {
  ...
  const ids = [
    ...
    "theme-select",
    "mode-select",
    "tournament-group-display",
    "tournament-title-override",
    ...
    "toggle-clock-btn",
    "clock-wrap",
    "toggle-penalty-mode-btn",
    "penalty-home-score",
    "penalty-away-score",
    "penalty-home-score-wrap",
    "penalty-away-score-wrap",
    "undo-last-penalty-btn",
    "ctrl-home-penalty-score",
    "ctrl-away-penalty-score",
    "penalty-tracker",
    "penalty-home-marks",
    "penalty-away-marks",
    "reset-scores-btn",
    "reset-teams-btn",
    "reset-all-btn",
    "toggle-team-names-btn",
    "clear-title-override-btn",
    "confirm-reset-all-btn",
    "confirm-start-time-btn",
    "clock-min",
    "clock-sec",
    "modal-overlay",
    "start-time-modal",
...
function setupListeners() {
  ...
  ui.newGameBtn?.addEventListener("click", () => {
    ...
    window.open(
      `${window.location.origin}${window.location.pathname}?id=${newId}`,
      "_blank",
    );
  });
  ...
  ["home", "away"].forEach((side) => {
    ...
    searchInput?.addEventListener("input", (e) =>
      debouncedSearch(side, e.target.value),
    );
    searchInput?.addEventListener("keydown", (e) =>
      handleSearchKeyboard(e, side),
    );
    searchInput?.addEventListener("focus", (e) => {
      if (!e.target.value.trim()) debouncedSearch(side, "");
    });
    ui[`${side}ClearSearchBtn`]?.addEventListener("click", () => {
      searchInput.value = "";
      debouncedSearch(side, "");
    });
    ui[`${side}NameOverride`]?.addEventListener("input", (e) =>
      overrideName(side, e.target.value),
    );
    ui[`${side}LogoUpload`]?.addEventListener("change", (e) =>
      handleLogoUpload(side, e.target),
    );
  });
  ...
  document
    ...
    .forEach((btn) => btn.addEventListener("click", closeActiveModal));

  // Overlay click to close when clicking outside modal-card
  document.querySelectorAll(".modal-overlay").forEach((ov) => {
    ov.addEventListener("click", (e) => {
      if (e.target === ov && ov.id !== "mobile-warning-modal")
        closeActiveModal();
    });
  });
}
...
document.addEventListener("click", (e) => {
  if (isMobileHeaderViewport()) {
    ...
    if (
      ...
      shellUiManager.setHeaderMenu(false);
    }
  }
  ...
  const isLocked =
    activeEl && ["SELECT", "INPUT", "TEXTAREA"].includes(activeEl.tagName);
  if (
    isLocked &&
